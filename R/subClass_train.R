...
subClass_train<-function(cnProc_broad, stTrain, expTrain, colName_broadCat, colName_subClass, name_broadCat, colName_samp, nTopGenes = 20, nTopGenePairs = 50, nRand = 40, nTrees = 1000, weightedDown_total = 5e5, weightedDown_dThresh = 0.25, transprop_xFact = 1e5) {
   ...
   system.time(cgenes<-findClassyGenes(expDat = expTnorm_sub, sampTab = stTrain_sub, dLevel = colName_cat, topX = nTopGenes))
   ...
   system.time(xpairs<-ptGetTop(expTnorm_sub[cgenesA,], grps, topX=50, sliceSize=2000))
   ...
   expValTrans = subClassQuery_transform(expDat = expTrain, cgenes = cgenes, xpairs = xpairs, classMatrix = classMatrix)
   ...
   returnList = list("expTnorm" = expTnorm, "cgenes_list" = cgenes, "cnProc_subClass" = cnProc_subClass)
